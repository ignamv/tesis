...
Otro tipo importante de ruido a considerar es la deriva en tiempos más largos.
Para esto, comparamos la tensión de salida de los APS un tiempo fijo después de su reseteo, para varios disparos.
El resultado (figuras~\ref{fig:comparacion_salida_disparos_aps4} y~\ref{fig:comparacion_salida_disparos_aps40}) es que hay una diferencia entre disparos comparable con la resolución del conversor A/D (unos \SI{39}{\milli\volt}).
...
La diferencia es comparable con la resolución del conversor A/D.}
...
La diferencia es comparable con la resolución del conversor A/D.}
Con un conversor de más resolución y mayor cantidad de disparos, sería posible caracterizar este número en detalle.
Sin embargo, alcanza para ver que la variación disparo a disparo es mucho mayor al ruido instantáneo visto en muestras sucesivas dentro de un disparo.
%
